...
export const AdminTaskListing: React.FC = () => {
  ...
  const toggleEmployee = (empId: number) => {
    setExpandedEmployees(prev => ({ ...prev, [empId]: prev[empId] === undefined ? false : !prev[empId] }));
  };
  ...
  const handleUpdateTaskStatus = async (projectId: number, taskId: number, newStatusId: number) => {
    try {
      await updateTask({
        projectId,
        taskId,
  ...
  const groupedTasks = useMemo(() => {
    ...
    projectsData.items.forEach(project => {
      project.tasks?.forEach(task => {
        ...
        if (search && !task.name.toLowerCase().includes(search.toLowerCase()) && !project.project_name.toLowerCase().includes(search.toLowerCase())) return;

        const empId = task.assignee?.id || 0; // 0 for unassigned
        ...
        if (!employeeMap.has(empId)) {
          employeeMap.set(empId, {
            ...
            tasks: [],
            completedCount: 0,
            initials: empName === 'Unassigned' ? '?' : empName.substring(0, 2).toUpperCase()
          });
        }
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl space-y-8 pb-20">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {metadata?.task_statuses?.map(s => (
                <option key={s.id} value={s.id}>{s.task_status}</option>
              ))}
            </select>
        ...
        {/* Grouped Tasks */}
        {isLoading ? (
          <div className="flex justify-center p-20"><div className="h-8 w-8 animate-spin rounded-full border-4 border-blue-500 border-t-transparent"></div></div>
        ) : (
          <div className="space-y-6">
            {groupedTasks.map(group => {
              const isExpanded = expandedEmployees[group.id] !== false; // default true
              return (
                <div key={group.id} className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden transition-all">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-semibold text-slate-500">
                          {group.tasks.length} Assigned Tasks &bull; {group.completedCount} Completed
                        </p>
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="divide-y divide-slate-100">
                      {group.tasks.map(task => (
                        <div key={task.id} className="flex items-center justify-between p-6 transition hover:bg-slate-50/50">
                          <div className="flex items-start gap-4">
                            {task.status?.name === 'Completed' ? (
                              <svg className="mt-0.5 h-5 w-5 text-[#10B981]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" /></svg>
                            ) : task.status?.name === 'In Progress' ? (
                              <svg className="mt-0.5 h-5 w-5 text-[#3B82F6]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                            ) : (
                          ...
                          <div>
                            <select 
                              ...
                              onChange={(e) => handleUpdateTaskStatus(task.project_id, task.id, Number(e.target.value))}
                            ...
                            >
                              {metadata?.task_statuses?.map(s => (
                                <option key={s.id} value={s.id} style={{ color: '#334155' }}>{s.task_status}</option>
                              ))}
                            </select>
                          </div>
                        </div>
          ...
          <div className="flex h-full flex-col">
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm font-semibold text-slate-500">Assign a new task to an employee</p>
              </div>
              <button onClick={() => setIsDrawerOpen(false)} className="rounded-full p-2 text-slate-400 hover:bg-slate-50 hover:text-slate-600">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <form id="task-form" onSubmit={handleCreateTask} className="space-y-6">
                <div>
                  <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-slate-500">Project</label>
                  <select required value={formProjectId} onChange={e => setFormProjectId(Number(e.target.value))} className="w-full rounded-lg border border-slate-300 px-4 py-2.5 bg-white text-sm font-medium text-slate-700 outline-none focus:border-[#3B82F6]">
                ...
                <div>
                  <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-slate-500">Task Name</label>
                  <input type="text" required value={formTaskName} onChange={e => setFormTaskName(e.target.value)} className="w-full rounded-lg border border-slate-300 px-4 py-2.5 bg-white text-sm font-medium text-slate-700 outline-none focus:border-[#3B82F6]" placeholder="e.g. Design UI Mockups" />
                </div>
                <div>
                  <label className="mb-2 block text-xs font-bold uppercase tracking-wider text-slate-500">Assignee</label>
                  <select value={formAssigneeId} onChange={e => setFormAssigneeId(e.target.value ? Number(e.target.value) : '')} className="w-full rounded-lg border border-slate-300 px-4 py-2.5 bg-white text-sm font-medium text-slate-700 outline-none focus:border-[#3B82F6]">
                ...
                <div>
                  ...
                  <select value={formStatusId} onChange={e => setFormStatusId(Number(e.target.value))} className="w-full rounded-lg border border-slate-300 px-4 py-2.5 bg-white text-sm font-medium text-slate-700 outline-none focus:border-[#3B82F6]">
                    {metadata?.task_statuses?.map(s => (
                      <option key={s.id} value={s.id}>{s.task_status}</option>
                    ))}
                  </select>
            ...
            <div className="border-t border-slate-100 bg-slate-50 p-6 flex gap-3">
              <button type="button" onClick={() => setIsDrawerOpen(false)} className="flex-1 rounded-lg border border-slate-200 bg-white py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50">Cancel</button>
              <button type="submit" form="task-form" className="flex-1 rounded-lg bg-gradient-to-r from-[#3B82F6] to-[#8B5CF6] py-3 text-sm font-bold text-white transition hover:opacity-90">Create Task</button>
            </div>
          </div>
        </div>
      </div>
