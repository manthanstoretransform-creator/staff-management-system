...
using `pynput` global listeners on both Windows and macOS — a dependency
adopted with explicit user sign-off (input_probe.py's own docstring calls
that "a deliberate product decision rather than something to adopt
silently").
...
class InputEventCounter:
    ...
    def start(self) -> bool:
        """Start both listeners. Returns True when they are running.

        Never raises: a missing pynput or an OS-level refusal marks the
    ...
    def stop(self) -> None:
        """Stop both listeners deterministically. Safe to call repeatedly."""
