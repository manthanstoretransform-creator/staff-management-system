...
| Input Monitoring | `NSInputMonitoringUsageDescription` | `pynput`'s global listeners produce keyboard/mouse **counts**. | macOS delivers no events; counts stay at zero and activity reports as unmeasured. No crash. |
...
- **macOS builds have not been produced or run by this project yet.** The
  packaging configuration, the build script, the entitlements, the
  `Info.plist` and the CI job exist and are complete; no macOS build has been
  executed, so nothing about macOS here is claimed as tested.
