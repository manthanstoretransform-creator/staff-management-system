...
def create_project(
    ...
    # TODO: Verify if docs/Project_Requirements.md implies a role/permission check (e.g. manage_projects).
    # Since it is currently unspecified, we require general authentication only.
...
@router.patch("/{id}/archive", response_model=ProjectRead)
def archive_project(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
