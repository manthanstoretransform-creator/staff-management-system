"""
Unit tests for the desktop activity pipeline's backend half:

- TimeEntryActivityService.batch_record_activity (keyboard/mouse windows)
- TimeEntryUnwantedActivityService.record_event / record_adjustment

All follow the same ownership/org validation contract as the app-usage
service: identity fields come from the authenticated user and the time
entry itself, never from the client, and client_event_id makes queued
retries idempotent.
"""
import unittest
...
class TestAdjustments(unittest.TestCase):
    ...
    def test_schema_rejects_positive_and_oversized_adjustments(self):
        with self.assertRaises(ValidationError):
            AdjustmentCreate(adjustment_seconds=600, reason="credit")   # positive
        with self.assertRaises(ValidationError):
